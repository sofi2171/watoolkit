...
<head>
    ...
    <style>
        ...
        .download-btn { position: absolute; bottom: 8px; right: 8px; background: #25d366; color: white; border: none; border-radius: 50%; width: 40px; height: 40px; font-size: 20px; font-weight: bold; box-shadow: 0 2px 5px rgba(0,0,0,0.5); display: flex; justify-content: center; align-items: center; z-index: 10;}
...
<body>
    ...
    <script>
        ...
        // 1. Native Android Popup ke liye function
        async function askPermission() {
            try {
                const fs = window.Capacitor.Plugins.Filesystem;
                // Yeh code Android ka native Allow/Deny popup show karega
                await fs.requestPermissions(); 
                checkAndLoad();
            } catch(e) {
                alert('Setting se manually allow karein (Storage Permission)');
            }
        }
        ...
        async function checkAndLoad() {
            ...
            try {
                ...
                // 2. Duplicate files ko khatam karne ka logic
                ...
                if(allStatuses.length === 0) {
                    container.innerHTML = '<h3 style="text-align:center; width:100%; color:#555;">No statuses found. Open WhatsApp first!</h3>';
                } else {
                    renderMedia();
                }
            } catch(e) {
        ...
        function renderMedia() {
            ...
            filtered.forEach(file => {
                ...
                // 3. Media box alag aur download button alag (Taa ke click mix na ho)
                ...
                // Download Button (stopPropagation isliye taa ke video na khule jab download dabayein)
        ...
        async function downloadMedia(uri, name) {
            try {
                ...
                alert('✅ Status Saved in Gallery/Downloads!');
            } catch(e) {
                alert('⚠️ Download Failed: ' + e.message);
            }
        }

        // 4. Video aur Image ko play karne ka mukammal function
